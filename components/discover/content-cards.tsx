...
export async function ContentCards({
  ...
  const surface =
    theme === "kids"
      ? "rounded-[var(--radius)] border border-sky-200/90 bg-white/85 p-8 shadow-[0_16px_48px_rgba(15,23,42,0.08)] backdrop-blur-md"
  ...
  return (
    ...
    >
      ...
      <p className={`text-sm ${mutedClass}`}>{t("subheading")}</p>
        ...
        {contents.map((item) => {
          ...
          return (
            <li key={item.id}>
              {item.content_type === "video" ? (
                <article
                  className={`group overflow-hidden rounded-[20px] border shadow-md transition hover:shadow-lg ${
                    theme === "kids"
                      ? "border-sky-200/80 bg-white/95 hover:border-sky-400/60"
                ...
                <article
                  className={`flex h-full flex-col justify-between rounded-[20px] border p-5 shadow-md transition hover:shadow-lg ${
                    theme === "kids"
                      ? "border-sky-200/80 bg-white/95 hover:border-sky-400/60"
